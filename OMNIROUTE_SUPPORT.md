...
| `POST /v1/audio/transcriptions` | Batch audio transcription (`registerMediaUnderstandingProvider`) | ✅ Initial support — explicit model, multipart upload, and bounded transcript validation; not realtime |
...
10. ~~Add batch transcription (STT)~~ ✅ Done: register via `registerMediaUnderstandingProvider`, map explicitly selected models to OmniRoute's multipart `POST /v1/audio/transcriptions`, preserve upload metadata through OpenClaw's public helper, and validate bounded transcript/error responses. This remains batch REST transcription, not realtime.
