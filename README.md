...
2. **No synthetic default** — The plugin does not hardcode `auto`, any other combo, or a fallback model. A model is shown only when OmniRoute advertises it; discovery failure does not fabricate a model selection. Omitted context and output limits stay unknown instead of being filled with guessed token counts.
3. **Catalog diagnostics** — The packaged `omniroute-catalog-audit` command shows the authenticated gateway's advertised metadata without filling gaps with plugin guesses. Discovery uses the same rule: missing sizing or thinking-tier metadata stays missing.
...
image-generation, video-generation, and web-search providers. The longer-term
