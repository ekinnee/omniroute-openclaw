...
function copyAdvertisedFields(entry: JsonRecord): Partial<Record<(typeof ADVERTISED_MODEL_FIELDS)[number], unknown>> {
  const advertised: Partial<Record<(typeof ADVERTISED_MODEL_FIELDS)[number], unknown>> = {};
...
function copyAdvertisedCapabilities(entry: JsonRecord): Partial<Record<(typeof ADVERTISED_CAPABILITY_FIELDS)[number], unknown>> {
  const capabilities: Partial<Record<(typeof ADVERTISED_CAPABILITY_FIELDS)[number], unknown>> = {};
...
function buildMissingFields(
  catalogClass: OmniRouteCatalogAuditModel["catalogClass"],
  advertised: Partial<Record<(typeof ADVERTISED_MODEL_FIELDS)[number], unknown>>,
  capabilities: Partial<Record<(typeof ADVERTISED_CAPABILITY_FIELDS)[number], unknown>>,
): string[] {
...
export function buildOmniRouteCatalogAuditReport(params: {
  ...
  const models: OmniRouteCatalogAuditModel[] = [];
...
export function formatOmniRouteCatalogAuditReport(report: OmniRouteCatalogAuditReport): string {
  const lines = [
  ...
  for (const model of report.models) {
    const metadata = [
      ...Object.entries(model.advertised).map(
        ([key, value]) => `${key}=${JSON.stringify(value)}`,
      ),
      ...Object.entries(model.capabilities).map(
        ([key, value]) => `capabilities.${key}=${JSON.stringify(value)}`,
      ),
    ];
