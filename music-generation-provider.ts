...
type MusicGenerationProvider = Parameters<OpenClawPluginApi["registerMusicGenerationProvider"]>[0];
type MusicGenerationRequest = Parameters<MusicGenerationProvider["generateMusic"]>[0];
type MusicGenerationResult = Awaited<ReturnType<MusicGenerationProvider["generateMusic"]>>;
type GeneratedMusicAsset = MusicGenerationResult["tracks"][number];

const DEFAULT_TIMEOUT_MS = 300_000;
const MAX_TRACK_COUNT = 8;
const MAX_TOTAL_TRACK_BYTES = 32 * 1024 * 1024;

type MusicResponseItem = {
  url?: unknown;
  b64_json?: unknown;
  mime_type?: unknown;
  format?: unknown;
...
function requireMusicModel(model: string): string {
  const normalized = model.trim();
  if (!normalized) {
    throw new Error(
      "OmniRoute music generation requires an explicit music model. Set the music generation model to a model advertised by OmniRoute's /v1/models endpoint.",
    );
  }
  return normalized;
}

function resolveConfiguredBaseUrl(req: MusicGenerationRequest): string {
  return resolveOmniRouteBaseUrl({ config: req.cfg });
}

function resolveMusicMimeType(item: MusicResponseItem): string | undefined {
...
function assertMusicBytes(buffer: Buffer, mimeType: string): void {
...
function resolveMusicFileName(item: MusicResponseItem, index: number, mimeType: string): string {
...
async function materializeMusicItem(
  item: MusicResponseItem,
  index: number,
  timeoutMs: number,
...
async function parseMusicResponse(
  ...
  const items = (payload as { data: unknown[] }).data.map((entry) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      throw new Error("OmniRoute music generation response malformed");
    }
    return entry as MusicResponseItem;
  });
  if (items.length === 0) {
    throw new Error("OmniRoute music generation response missing audio data");
  }
...
export function buildOmniRouteMusicGenerationProvider(): MusicGenerationProvider {
  return {
    ...
    async generateMusic(req) {
      ...
      try {
        ...
        const tracks = await parseMusicResponse(
          await readOmniRouteJson(
            request.response,
            "omniroute.music-generation",
            OMNIROUTE_JSON_READ_OPTIONS.musicGeneration,
          ),
          timeoutMs,
        );
        return { tracks, model };
