// OmniRoute web fetch provider using the public OpenClaw web-fetch contract.
...
function readProviderConfig(config: OpenClawConfig | undefined): Record<string, unknown> | undefined {
  return readRecord(
    readRecord(readRecord(config)?.models)?.providers,
  )?.[OMNIROUTE_PROVIDER_ID] as Record<string, unknown> | undefined;
}

function readFetchConfig(config: OpenClawConfig | undefined): Record<string, unknown> | undefined {
  return readRecord(
    readRecord(readRecord(config)?.tools)?.web,
  )?.fetch as Record<string, unknown> | undefined;
}
...
async function resolveWebFetchApiKey(params: {
  ...
  try {
    ...
    if (!isMissingOmniRouteAuthError(error)) throw error;
  }
...
function resolveTimeoutMs(fetchConfig: Record<string, unknown> | undefined): number {
...
async function assertWebFetchOk(response: Response): Promise<void> {
  ...
  try {
    const payload = await readOmniRouteJson(
      response,
      "omniroute.web-fetch.error",
      OMNIROUTE_JSON_READ_OPTIONS.webFetchError,
    );
  ...
  throw new Error(
    `OmniRoute web fetch failed: HTTP ${response.status}${detail ? `: ${detail}` : ""}`,
  );
}

export function createOmniRouteWebFetchProvider(): WebFetchProviderPlugin {
  return {
    ...
    getConfiguredCredentialValue: (config) => {
      ...
      if (fetchConfig && Object.hasOwn(fetchConfig, "apiKey")) return undefined;
    ...
    createTool: (ctx) => {
      ...
      return {
        description:
          "Fetch and extract a URL through OmniRoute. Returns bounded readable content " +
        ...
        execute: async (args) => {
          ...
          try {
            ...
            const payload = readRecord(await readOmniRouteJson(
              request.response,
              "omniroute.web-fetch",
              OMNIROUTE_JSON_READ_OPTIONS.webFetch,
            ));
