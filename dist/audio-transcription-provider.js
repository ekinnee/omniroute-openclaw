...
function requireExplicitModel(model) {
    ...
    if (!normalized) {
        throw new Error("OmniRoute audio transcription requires an explicitly selected model");
    }
    return normalized;
}
function resolveAudioBaseUrl(baseUrl) {
    ...
    return resolveOmniRouteBaseUrl({
        ...
        overrideBaseUrl: normalized && normalized !== OMNIROUTE_DEFAULT_BASE_URL ? normalized : undefined,
    });
}
async function transcribeOmniRouteAudio(params) {
...
export const omniRouteMediaUnderstandingProvider = {
    id: OMNIROUTE_PROVIDER_ID,
    capabilities: ["audio"],
    transcribeAudio: transcribeOmniRouteAudio,
};
//# sourceMappingURL=audio-transcription-provider.js.map